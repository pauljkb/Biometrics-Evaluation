...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.num_register_token > 0:
            self.register_tokens = nn.Parameter(torch.zeros(1, self.num_register_token, embed_dim))
            self.num_patches = self.num_patches + self.num_register_token
            print(f"Number of images patches {self.num_patches} + Number of register tokens {self.num_register_token}")


        self.pos_embed = nn.Parameter(torch.zeros(1, (self.num_patches-self.num_register_token), embed_dim)) # Register token do NOT require postional encoding
        ...
        self.feature = nn.Sequential(
            nn.Linear(in_features=embed_dim * (self.num_patches-self.num_register_token), out_features=embed_dim, bias=False),
    ...
    def forward_features(self, x):
        ...
        if self.num_register_token > 0:
            num_img_patches = self.num_patches - self.num_register_token
            return torch.reshape(x[:,:num_img_patches], (B, num_img_patches * self.embed_dim))
